...
#     def train_classifier(self):
#         data_dir = r"C:\Users\jiyac\OneDrive\Documents\MINIPROJECT\COLLEGE_IMAGES\data"
...
#             try:
#                 file_name = os.path.split(image_path)[1]
#                 id = int(file_name.split('.')[1])  # Extract the ID from the filename
#                 img = Image.open(image_path).convert('L')  # Convert to grayscale
#                 image_np = np.array(img, 'uint8')

#                 faces.append(image_np)
#                 ids.append(id)
#                 print(f"Training with ID: {id}, Image Path: {image_path}")
...
#             clf = cv2.face.LBPHFaceRecognizer_create()
#             clf.train(faces, ids)
#             model_path = r"C:\Users\jiyac\OneDrive\Documents\MINIPROJECT\classifier.xml"
#             clf.write(model_path)
#             cv2.destroyAllWindows()
#             messagebox.showinfo("Result", f"Training completed successfully. Model saved at {model_path}.")
#         except Exception as e:
